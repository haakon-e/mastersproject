...
def test_biot_parameter_scaling(**kw):
    ...
    params = {
        ...
        "source_scalar_borehole_shearzone": {"borehole": "INJ1", "shearzone": "S1_1"},
...
def test_biot_condition_number(
    ...
    _folder_root = (
        f"{this_method_name}/{now_as_YYMMDD}/{test_name}/{shearzone_str}/{scale_str}"
    )
...
def test_biot_solution_scaling(test_name: str, ls: float, ss: float):
    ...
    * No shear zones
    ...
    params = test_util.prepare_params(
        path_head=_folder_root, params=base_params.copy(), setup_loggers=True
    )
    ...
    params = test_util.prepare_params(
        path_head=_folder_root, params=params.copy(), setup_loggers=False
    )
