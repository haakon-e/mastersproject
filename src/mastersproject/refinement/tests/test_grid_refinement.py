...
class TestGridRefinement:
    ...
    def test_refine_mesh_by_splitting(self):
        ...
        assert gb.num_cells() == gb_list[0].num_cells(), "The first grid should correspond to the initial grid"
        num_cells = [_gb.num_cells() for _gb in gb_list]
        for i in range(1, n_grids):
            assert num_cells[i - 1] < num_cells[i], "Each subsequent grid should be a refinement of the previous grid"
    ...
    def test_coarse_fine_cell_mapping(self):
        ...
        # Assertions
        assert coarse_fine.indices.size == g_ref.num_cells, "Every fine cell should be inside exactly one coarse cell"
    ...
    def test_gb_coarse_fine_cell_mapping(self):
        ...
        for g, data in gb:
            ...
            g_ref_lst = gb_ref.get_grids(lambda g: gb_ref.node_props(g, 'node_number') == node_num)
            assert len(g_ref_lst) == 1, "There should be exactly one corresponding grid"
            g_ref = g_ref_lst[0]

            coarse_fine = data[key]
            assert coarse_fine.shape[0] == g_ref.num_cells, "Check the shape of the coarse_fine cell mapping"
            assert coarse_fine.shape[1] == g.num_cells, "Check the shape of the coarse_fine cell mapping"


def generate_n_grids(
        path_head: str,
        n_grids: int
) -> List[pp.GridBucket]:
...
def create_gb_with_simple_fracture(path_head: str) -> Tuple[pp.FractureNetwork3d, Path, pp.GridBucket]:
    """ Setup method.
